const CACHE='giostacchio-lavagna-v03';
const ASSETS=['./','./index.html','./manifest.webmanifest','../icon-192.png','../icon-512.png','../apple-touch-icon.png'];
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(ASSETS)).then(()=>self.skipWaiting())));
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE&&k.startsWith('giostacchio-lavagna-')).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
